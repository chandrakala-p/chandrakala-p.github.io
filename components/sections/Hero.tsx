...
import { Github, Linkedin, Mail, Sparkles, ArrowRight, BookOpen } from 'lucide-react';
...
const techStack = [
  ...
  'PostgreSQL', 'MongoDB', 'Redis', 'Docker', 'React',
];
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-24">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          ...
          >
            ...
            <motion.div variants={itemVariants} className="flex items-center gap-3 mb-5 justify-center lg:justify-start flex-wrap">
            ...
            <motion.div variants={itemVariants} className="flex items-center gap-3 justify-center lg:justify-start">
              ...
              <span className="text-xs text-slate-400 dark:text-slate-600 ml-2">{personal.location}</span>
            </motion.div>
          </motion.div>

          {/* Right — clean avatar + tech stack (no overlapping badges) */}
          <motion.div
          ...
          >
            <div className="relative w-52 h-52 sm:w-64 sm:h-64">
              ...
              <div className="absolute inset-6 rounded-full bg-gradient-to-br from-emerald-400 via-emerald-500 to-teal-600 shadow-emerald-glow-lg flex items-center justify-center">
            ...
            {/* Tech chips — below avatar, no overlap */}
            <div className="flex flex-wrap justify-center gap-2 max-w-[260px] sm:max-w-[300px]">
              {techStack.map((tech, i) => (
                <motion.div
                  key={tech}
                  initial={{ opacity: 0, scale: 0.8 }}
                  animate={{ opacity: 1, scale: 1 }}
                  transition={{ delay: i * 0.07 + 0.9, duration: 0.3, ease: 'backOut' }}
                >
                  <Badge variant="emerald" size="sm" className="shadow-sm font-medium">
                    {tech}
                  </Badge>
                </motion.div>
              ))}
        ...
        >
          {personal.stats.map((stat) => (
            ...
            >
              ...
              <p className="mt-1 text-xs font-medium text-slate-500 dark:text-slate-500">{stat.label}</p>
            </div>
          ))}
        </motion.div>
      </div>

      {/* Scroll indicator — professional mouse icon, hidden on small screens */}
      <motion.button
        onClick={() => handleScroll('#about')}
        className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 hidden sm:flex flex-col items-center gap-1.5 text-slate-400 dark:text-slate-600 hover:text-emerald-500 dark:hover:text-emerald-400 transition-colors duration-200"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 2 }}
